...
#Importing the multi-processing module
...
if __name__ == "__main__":
    ...
    for col in range(SIZE):  #checking all columns
        #creat new process, ensure non-daemonic
        p1=mp.Process(target=checkColumn, args=(testcase,col), daemon=False)
        #append the process to a list of processes
        processes.append(p1)
        #start the process
        p1.start()
    ...
    for row in range(SIZE):  #checking all rows
        #creat new process, ensure non-daemonic
        p2=mp.Process(target=checkRow, args=(testcase,row), daemon=False)
    ...
    for subgrid in range(SIZE):   #checking all subgrids
        #creat new , ensure non-daemonic
        p3=mp.Process(target=checkSubgrid, args=(testcase,subgrid), daemon=False)
        #append the process to the new process
        processes.append(p3)
        #Start the process
        p3.start()

    for process in processes:   #for all the process in process, wait to ensure all processes are finished before terminated by the main
        #ensure child process is finished before terminated by main
        process.join()